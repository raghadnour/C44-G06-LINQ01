	...
	internal static class ListGenerator
	{
		...
		static ListGenerator()
		{
			...
			CustomersList = (from e in XDocument.Load("Customers.xml").Root?.Elements("customer")
							 select new Customer()
							 {
								 CustomerID = (string?)e.Element("id")??"Notfound",
								 CustomerName = (string?)e.Element("name") ?? "Notfound",
								 Address = (string?)e.Element("address") ?? "Notfound",
								 City = (string?)e.Element("city") ?? "Notfound",
								 Region = (string?)e.Element("region") ?? "Notfound",
								 PostalCode = (string?)e.Element("postalcode") ?? "Notfound",
								 Country = (string?)e.Element("country") ?? "Notfound",
								 Phone = (string?)e.Element("phone") ?? "Notfound",
								 Fax = (string?)e.Element("fax") ?? "Notfound",
								 Orders = (
